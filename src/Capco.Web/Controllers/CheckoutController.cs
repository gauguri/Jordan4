...
public class CheckoutController : Controller
{
    ...
    {
        var cart = await _cartService.GetOrCreateCartAsync(User.Identity?.Name, Request.Cookies["capco-cart"]);
        if (!cart.Items.Any())
        {
    ...
    public IActionResult Confirmation(string orderNumber)
    {
        ...
        var order = new Order { OrderNumber = orderNumber, Total = 0m, Status = "pending", Email = User.Identity?.Name ?? "guest@capco.local", Subtotal = 0m, Shipping = 0m, Tax = 0m, CreatedAt = DateTime.UtcNow };
